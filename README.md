...
- **Quét mã QR** — camera hoặc upload ảnh QR chứa URL.
...
- Sau nâng cấp: `User.IsPremium = true`; navbar (`NavUserInfoViewComponent`) và `/Profile` hiển thị trạng thái VIP.
...
- Leaflet.js, Chart.js, html5-qrcode
...
│   └── ...
├── Helpers/
│   └── AuthSignInHelper.cs          # Claims cookie khi đăng nhập (gồm IsPremium)
├── ViewComponents/
│   └── NavUserInfoViewComponent.cs  # Navbar: email + badge VIP (đọc DB)
├── Views/
│   ├── UrlScanner/
│   ├── Premium/
│   ├── Profile/
│   ├── ScamReport/
│   ├── QrScan/
│   └── Shared/                      # _Layout, Components/NavUserInfo, _Chatbox
├── wwwroot/
├── Migrations/
├── Program.cs
├── appsettings.json
